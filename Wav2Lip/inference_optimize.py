...
parser.add_argument(
    ...
    default=False,
    type=bool,
    help="load from facial recognition and video parsing step Cache ",
)
...
def face_detect_video(frames):
    if args.box[0] == -1:
        if not args.static:
            # BGR2RGB for CNN face detection
            face_det_results = face_detect(frames)
        else:
            frame = face_detect([frames[0]])[0][1]
            y1, y2, x1, x2 = frame
            face_det_results = [[f[y1:y2, x1:x2], frame] for f in frames]
    else:
...
def wave2Lip(audio_path, full_frames, fps, face_det_results, model):
    ...
    command = "ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}".format(
        audio_path, "temp/result.avi", args.outfile
    )
    subprocess.call(command, shell=platform.system() != "Windows")


# pre-compute
def process_video(video_path, crop, rotate, resize_factor):
...
def main():
    ...
    if args.cache:
        ...
        # pre compute
        full_frames, fps = process_video(
            args.face, args.crop, args.rotate, args.resize_factor
        )
        face_det_results = face_detect_video(full_frames)
        cache = {
    ...
    wave2Lip(
        args.audio, cache["full_frames"], cache["fps"], cache["face_det_results"], model
    )


def generate_video(
    audio, face, model, crop=[0, -1, 0, -1], rotate=False, resize_factor=1
):
    if args.cache:
        with open("cache.pkl", "rb") as f:
            cache = pickle.load(f)
        ...
        # pre compute
        full_frames, fps = process_video(face, crop, rotate, resize_factor)
        face_det_results = face_detect_video(full_frames)
        cache = {
            "full_frames": full_frames,
            "fps": fps,
        ...
        with open("cache.pkl", "wb") as f:
            pickle.dump(cache, f)
    wave2Lip(
        audio, cache["full_frames"], cache["fps"], cache["face_det_results"], model
    )


if __name__ == "__main__":
    # cache and precompute steps
    main()
